...
import carla, random, time, datetime, os, tensorflow as tf
import os
import cv2
...
class Menu():
    ...
    def run_1():
        ...
        message('How much steps do you wanna run per epoch? The default is 94.')
        steps = int(input('Enter steps >> ') or 94)
        message('How many steps do you wanna run for validation? The default is 23.')
        steps_val = int(input('Enter steps >> ') or 23)
        ...
        height = int(input('Enter height (default 66) >> ') or 66)
        message('Enter the training batch size')
        training_batch = int(input('Training batch (default 10000) >> ') or 10000)
        ...
        while valid:
            ...
            data = Data(int(width), int(height), count, training_batch)
            message('Data collection finished')

            if len(data.data) < training_batch:
                valid = False
    ...
    def run_2():
        ...
        sim_time = int(input('Enter the time you need the generator to run for (in minutes) >> '))
        clear()
        
        collector = Collector(world, sim_time)
    ...
    def run_3():
        ...
        if len(models) <= 0:
            ...
            try:
                ...
                cv2.imwrite(f"app_images/3.png",frame.image)
            except:
                raise PilotError("That didn't work. The path you entered must be wrong. Start again...")
            predictions = PilotNet(200, 66, predict=True).predict(frame, given_model=model)
            #clear()
            message('Predictions are...')
            #print(predictions)
            message(f'Steering angle: {predictions[0][0]}')
